...
# var_representation tests don't even involve Fibers
PHP_CONFIGURE_ARGS="--disable-all --disable-fiber-asm --enable-zts --enable-debug"
if [[ "$ARCHITECTURE" == i386 ]]; then
	PHP_IMAGE="$ARCHITECTURE/php"
	DOCKER_IMAGE_VALGRIND="var_representation-valgrind-test-runner:$ARCHITECTURE-$PHP_VERSION_FULL"
	CFLAGS="$CFLAGS -m32"
else
	PHP_IMAGE="php"
	DOCKER_IMAGE_VALGRIND="var_representation-valgrind-test-runner:$ARCHITECTURE-$PHP_VERSION"
fi

DOCKER_IMAGE_VALGRIND=var_representation-valgrind-test-runner:$PHP_VERSION_FULL
docker build \
